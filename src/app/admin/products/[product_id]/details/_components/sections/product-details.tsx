...
export default function ProductDetailsSection({ data }: { data: Product|null }) {
  return (
      ...
      <div className="grid grid-cols-1 gap-y-5 gap-x-7 sm:grid-cols-2 mb-5 items-baseline">
        <Label className="flex flex-col gap-y-2 text-shade text-[12px]">
          ...
          <div className="p-3 border rounded-sm text-black text-sm font-normal">
            {data?.box_details || "No Set"}
          </div>
        </Label>
        <Label className="flex flex-col gap-y-2 text-shade text-[12px]">
          Max Purchase Quantity
          <div className="p-3 border rounded-sm text-black text-sm font-normal">
            {data?.max_purchase_quantity || "No Set"}
          </div>
        </Label>
        <Label className="flex flex-col gap-y-2 text-shade text-[12px]">
          Model Height
          <div className="p-3 border rounded-sm text-black text-sm font-normal">
            {data?.model_height || "No Set"}
          </div>
        </Label>
        <Label className="flex flex-col gap-y-2 text-shade text-[12px]">
          Model Name
          <div className="p-3 border rounded-sm text-black text-sm font-normal">
            {data?.model_name || "No Set"}
          </div>
        </Label>
        <Label className="flex flex-col gap-y-2 text-shade text-[12px]">
          Quantity In Stouk
          <div className="p-3 border rounded-sm text-black text-sm font-normal h-[200px]">
            {data?.quantity_in_stock || "No Set"}
          </div>
        </Label>
        <Label className="flex flex-col gap-y-2 text-shade text-[12px]">
          Colour
